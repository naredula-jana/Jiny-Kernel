...
#define up(irq_flags,a) \
do {\
	spin_unlock_irqrestore(&mem_slab_lock, irq_flags);\
}while(0)
 ...
 * With mm_free(), these are used to find the cache which an obj belongs to.
 */
...
//TODO 	init_MUTEX(&cache_chain_sem);
	...
	ut_printf("kmem_cache_init done :%d \n",cache_cache.num);
}
...
void __init kmem_cache_sizes_init(void)
{
	...
	char name[20];
ut_printf(" In the kmem_cache_sizes_init \n");
	/*
	 * Fragmentation resistance on low memory - only use bigger
	 * page orders on machines with more than 32MB of memory.
	 */
	//TODO if (num_physpages > (32 << 20) >> PAGE_SHIFT)
	...
	//TODO 	snut_printf(name, sizeof(name), "size-%Zd(DMA)",sizes->cs_size);
...
static inline void * kmem_getpages (kmem_cache_t *cachep, unsigned long flags)
{
	...
	addr = (void*) mm_getFreePages(flags, cachep->gfporder);
...
static inline void kmem_freepages (kmem_cache_t *cachep, void *addr)
{
	...
	mm_putFreePages((unsigned long)addr, cachep->gfporder);
}
...
static inline void kmem_poison_obj (kmem_cache_t *cachep, void *addr)
{
	...
	ut_memset(addr, POISON_BYTE, size);
	...
	void (*dtor)(void*, kmem_cache_t *, unsigned long))
{
	...
	if (!cachep)
		goto opps;
	ut_memset(cachep, 0, sizeof(kmem_cache_t));
...
next:
	...
	down(irq_flags,&cache_chain_sem);
	{
		struct list_head *p;
		...
		/*	if (!strcmp(pc->name, name)) TODO: implementing sut_printf to correctly update the name 
				BUG();*/
		}
	}
...
static void drain_cpu_caches(kmem_cache_t *cachep)
{
	...
	ut_memset(&new.new,0,sizeof(new.new));
...
# define CHECK_NR(pg)						\
	do {							\
		if (!VALID_PAGE(pg)) {				\
			printk(KERN_ERR "mm_free: out of range ptr %lxh.\n", \
				(unsigned long)objp);		\
			BUG();					\
		} \
	} while (0)
# define CHECK_PAGE(page)					\
	do {							\
		CHECK_NR(page);					\
		if (!PageSlab(page)) {				\
			printk(KERN_ERR "mm_free: bad ptr %lxh.\n", \
				(unsigned long)objp);		\
			BUG();					\
		}						\
	} while (0)
 ...
 *
 * mm_malloc is the normal method of allocating memory
 * in the kernel.
 *
...
/**
 * mm_free - free previously allocated memory
 * @objp: pointer returned by mm_malloc.
 *
 * Don't free memory not originally allocated by mm_malloc()
...
static int kmem_tune_cpucache (kmem_cache_t* cachep, int limit, int batchcount)
{
	...
	if (limit) {
		for (i = 0; i< smp_num_cpus; i++) {
			...
			ccnew = mm_malloc(sizeof(void*)*limit+
...
oom:
	for (i--; i >= 0; i--)
		mm_free(new.new[cpu_logical_map(i)]);
	return -ENOMEM;
}
...
static int s_show(struct seq_file *m, void *p)
{
	...
	seq_ut_printf(m, "%-17s %6lu %6lu %6u %4lu %4lu %4u",
...
#if STATS
	{
		...
		seq_ut_printf(m, " : %6lu %7lu %5lu %4lu %4lu",
...
#if STATS && defined(CONFIG_SMP)
	{
		...
		seq_ut_printf(m, " : %6lu %6lu %6lu %6lu",
