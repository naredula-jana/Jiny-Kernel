...
static void parse_cpu(void *addr)
{
	...
	ut_printf("mptables: cpu lapic id:%x \n",cpu->lapic_id);
...
static void parse_ioapic(void *addr)
{
	...
	ut_printf("Mptables IO apic id: %x base:%x \n",ioapic->id,ioapic->base);
...
static void parse_irq(void *addr)
{
	...
	ut_printf("mptables : irq :%x  busirq:%x ioapicid:%x ioapicpin:%x type:%x\n",irq,irq->src_busirq,irq->dst_ioapicid,irq->dst_ioapicpin,irq->type);
...
void Jcmd_mptables_init(void)
{
	...
	ut_printf(" phy addr mpf:%x :%x \n,",mpf,mpf->conf_physaddr);
	//mpc  = vm_create_kmap("smp_apic",0x100000,PROT_WRITE,MAP_FIXED,mpf->conf_physaddr);
	//mpc = vm_kmap(mpf->conf_physaddr, sizeof(*mpc));
	mpc = __va(mpf->conf_physaddr);
ut_printf(" mpc:%x \n",mpc);
mpc_dump(mpc);
	if (!mpc_check(mpc)) {
		mpc_dump(mpc);
