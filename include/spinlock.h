...
static inline void arch_spinlock_lock(spinlock_t *lock, int line) {
	register struct task_struct *current_task = g_current_task;
#ifdef SPINLOCK_DEBUG
#ifdef RECURSIVE_SPINLOCK
	if (lock->recursion_allowed==1 && lock->pid == current_task->task_id  && current_task->task_id!=0) {
		lock->recursive_count++;
		lock->stat_recursive_locks++;
		return;
	}else{
		if (lock->recursion_allowed==0 && lock->pid == current_task->task_id){ /* already owning the lock */
			SPIN_BUG();
		}
	}
#endif
	...
	lock->log[lock->log_length].task_id = current_task->task_id;
	lock->log[lock->log_length].cpuid = current_task->current_cpu;
	lock->log[lock->log_length].name = current_task->name;
	lock->log[lock->log_length].spins = 1 + (lock->stat_count/10);
	lock->log[lock->log_length].index = lock->stat_locks;
	//lock->log[lock->log_length].line = line;
	lock->log_length++;
#endif
	lock->pid = current_task->task_id;
	current_task->locks_nonsleepable++;
